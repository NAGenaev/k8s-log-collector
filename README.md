...
2. Для каждой пары параллельно: тянет свой k8s deploy-токен из Jenkins Credentials, собирает одноразовый kubeconfig, находит поды деплоймента по `spec.selector.matchLabels`, забирает `kubectl logs` для каждого пода/контейнера (+ `--previous` при рестартах), считает `ERROR`/`WARN` и подсвечивает их (красный/жёлтый) через ANSI-эскейпы.
3. После завершения всех веток — сводный отчёт: сначала статистика (по поду+контейнеру, подытог по поду, общий тотал), затем сами цветные логи по каждой поде отдельно.
